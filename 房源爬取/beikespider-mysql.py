...
# 建立数据库连接
db = pymysql.Connect(host="localhost", port=3306, user="root", passwd="mysql123", db="bkhome", charset="utf8")
cursor = db.cursor()
...
# <li class="list-item" data-from="">网页分析
    ...
    # with open('贝壳二手房源信息.csv', 'a+', encoding='UTF-8', newline='') as csvfile:
    ...
    for house in house_list:
        ...
        temp = [name, address, clean_houseinfo, price, untiPrice, house_tag, house_url]
        print(temp)


        time.sleep(0.5)
        # 写入表格（贝壳二手房源信息.csv）
        sql = "insert into bkwuhan(name, address, houseinfo, price, untiPrice, house_tag, house_url) values('"+name+"', '"+address+"','"+clean_houseinfo+"','"+price+"','"+untiPrice+"','"+house_tag+"','"+house_url+"')"
        cursor.execute(sql)
        db.commit()
        # w.writerow(temp)
